...
const ItemDetailPage = () => {
  ...
  const handleBid = () => {
    ...
    if (isNaN(amount) || amount <= item.currentBid) {
      setBidError(`Bid must exceed current bid of Rs.${item.currentBid.toLocaleString()}`);
      return;
    }
    setBidError("");
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40 }}>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ type: "spring", delay: 0.1 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 28 }}>
              {[
                { label: "Current Bid", value: `Rs.${item.currentBid.toLocaleString()}`, color: "#F5C542" },
                { label: "Starting Price", value: `Rs.${item.startPrice.toLocaleString()}`, color: "var(--text-primary)" },
                { label: "Reserve Price", value: `Rs.${item.reservePrice.toLocaleString()}`, color: "#2DD4BF" },
            ...
            {!isSold ? (
              <div>
                <div style={{ marginBottom: 12 }}>
                  <input type="number" value={bidAmount} onChange={e => setBidAmount(e.target.value)}
                    placeholder={`Enter bid > Rs.${item.currentBid.toLocaleString()}`} style={{ borderRadius: 12 }} />
                  {bidError && (
              ...
              <div style={{ padding: "20px", borderRadius: 14, textAlign: "center", background: "rgba(251,113,133,0.08)", border: "1px solid rgba(251,113,133,0.25)" }}>
                ...
                <div style={{ color: "var(--text-secondary)", fontSize: 13, marginTop: 4 }}>Final price: Rs.${item.currentBid.toLocaleString()}</div>
              </div>
            )}
          </motion.div>
        </div>
